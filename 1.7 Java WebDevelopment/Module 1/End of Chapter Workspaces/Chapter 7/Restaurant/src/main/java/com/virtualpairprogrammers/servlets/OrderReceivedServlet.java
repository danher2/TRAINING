...
public class OrderReceivedServlet extends HttpServlet {
	...
	public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// to create cookies in the session 
		HttpSession session = request.getSession();
		session.setAttribute("total", total);
		
		//when user avoid cookies
		String redirectUrl = "/thankYou.html";
		redirectUrl = response.encodeURL(redirectUrl); // the session will appear in the url
		response.sendRedirect(redirectUrl);// the url is made and shown in the browser
