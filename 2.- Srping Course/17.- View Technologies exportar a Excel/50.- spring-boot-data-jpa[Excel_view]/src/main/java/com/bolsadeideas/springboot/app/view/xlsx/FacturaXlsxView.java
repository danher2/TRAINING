...
public class FacturaXlsxView extends AbstractXlsxView {
	...
	// metodo donde se construye el documento Excel
	
	@Override
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		
		// encabezado de la respuesta  - devuelve el archivo en excel
		response.setHeader("Content-Disposition", "attachment; filename=\"factura_view.xlsx\"");
		Factura factura = (Factura) model.get("factura"); // obtenemos la factura que se envia desde el controlador y la casteamos a objeto factura
		
		//comienza el diseno de factura en Excel
		
		// creamos la hoja de excel
		Sheet sheet = workbook.createSheet("Factura Spring"); // nombre de la hoja
		
		//para traduir a varios idiomas
		...
		int rownum = 10; // partimos desde la fila 10
		
		
		//creamos celda por cada atributo
		for(ItemFactura item: factura.getItems()) { // recorremos la lista
			//por cada item crea una fila nueva (rownum ++) -> le agrega 1 al ultimo numero de fila
